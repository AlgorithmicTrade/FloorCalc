...
run_preflight_checks() {
    ...
    if ! git diff-index --quiet HEAD -- 2>/dev/null; then
        ...
        # Skip commit if nothing was staged (e.g. only submodule dirty state)
        if git diff --cached --quiet; then
            log_info "Nothing to commit (only submodule state changes, skipping auto-commit)"
        else
...
Co-Authored-By: Claude <noreply@anthropic.com>"
        ...
        # Create commit (bypass broken beads hooks by overriding hooksPath)
        git -c core.hooksPath=.git/hooks commit -m "$COMMIT_MSG" >/dev/null 2>&1 || {
        ...
        fi  # end of "else" block for staged changes check
    fi
    ...
    # Get current version
    # On Windows (Git Bash), cygpath converts Unix-style path to Windows path for Node.js
    local NODE_PROJECT_ROOT="$PROJECT_ROOT"
    if command -v cygpath &> /dev/null; then
        NODE_PROJECT_ROOT="$(cygpath -w "$PROJECT_ROOT")"
    fi
    CURRENT_VERSION=$(node -p "require('${NODE_PROJECT_ROOT//\\/\\\\}/package.json').version")
...
format_changelog_line() {
    ...
    # Append commit body if present (strip release automation boilerplate)
    local body
    body=$(git show --format="%b" --no-patch "$hash" 2>/dev/null | \
        awk '/^Auto-committed [0-9]+|^Files changed:|^Co-Authored-By:/{exit} {print}' | \
        sed 's/[[:space:]]*$//')

    # Include body only if it has meaningful content (not just blank lines)
    local body_trimmed
...
update_package_files() {
    ...
    while IFS= read -r pkg; do
        if [ -n "$pkg" ]; then
            ...
            # Convert to Windows path for Node.js on Windows (Git Bash)
            local node_pkg="$pkg"
            if command -v cygpath &> /dev/null; then
                node_pkg="$(cygpath -w "$pkg")"
            fi
            local node_pkg_escaped="${node_pkg//\\/\\\\}"

            # Update version using Node.js for proper JSON handling
            node -e "
                const fs = require('fs');
                const path = '${node_pkg_escaped}';
...
update_changelog() {
    ...
    if [ -f "$changelog_file" ]; then
        ...
        # Use grep/awk directly on file to avoid SIGPIPE from echo|grep -q pattern
        if grep -q "## \[Unreleased\]" "$changelog_file"; then
            # Find the line number of [Unreleased]
            local unreleased_line
            unreleased_line=$(grep -n "## \[Unreleased\]" "$changelog_file" | awk -F: 'NR==1{print $1}')

            # Insert after [Unreleased] and its blank line using awk directly on file
            {
                awk -v n="$unreleased_line" 'NR <= n {print}' "$changelog_file"
                echo ""
                echo "$new_entry"
                awk -v skip="$unreleased_line" 'NR > skip {print}' "$changelog_file"
            } > "${changelog_file}.tmp" && mv "${changelog_file}.tmp" "$changelog_file"
        else
            # No [Unreleased] section, insert at the beginning after header
            {
                awk 'NR <= 6 {print}' "$changelog_file"
                echo ""
                echo "$new_entry"
                awk 'NR > 6 {print}' "$changelog_file"
            } > "${changelog_file}.tmp" && mv "${changelog_file}.tmp" "$changelog_file"
...
update_release_notes() {
    ...
    if [ -f "$release_notes_file" ]; then
        ...
        if grep -q "^# Release Notes" "$release_notes_file"; then
            ...
            first_release_line=$(grep -n "^## v" "$release_notes_file" | awk -F: 'NR==1{print $1}')

            if [ -n "$first_release_line" ] && [ "$first_release_line" -gt 0 ]; then
                # Insert new entry before the first release
                {
                    awk -v n="$((first_release_line - 1))" 'NR <= n {print}' "$release_notes_file"
                    echo "$new_entry"
                    echo ""
                    awk -v skip="$((first_release_line - 1))" 'NR > skip {print}' "$release_notes_file"
                } > "${release_notes_file}.tmp" && mv "${release_notes_file}.tmp" "$release_notes_file"
            else
                # No releases yet, append after header
                {
                    awk 'NR <= 4 {print}' "$release_notes_file"
                    echo ""
                    echo "$new_entry"
                } > "${release_notes_file}.tmp" && mv "${release_notes_file}.tmp" "$release_notes_file"
            fi
            ...
            existing_content=$(<"$release_notes_file")
            {
                cat << EOF
# Release Notes
...
📌 Version: $CURRENT_VERSION → $NEW_VERSION ($(echo "$BUMP_TYPE" | tr '[:lower:]' '[:upper:]'))
...
execute_release() {
    ...
    git -c core.hooksPath=.git/hooks commit -m "chore(release): v$NEW_VERSION
...
$(generate_changelog_entry "$NEW_VERSION" "$DATE")"
    ...
    # Write tag message to a temp file to avoid "Argument list too long" on Windows
    local tag_msg_file
    tag_msg_file=$(mktemp)
    printf '%s' "$tag_message" > "$tag_msg_file"
    ...
    # Push to remote (bypass broken beads hooks by overriding hooksPath)
    log_info "Pushing to remote..."
    git -c core.hooksPath=.git/hooks push origin "$BRANCH" --follow-tags || {
