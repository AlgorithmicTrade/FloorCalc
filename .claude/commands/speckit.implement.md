...
> **EXECUTION CONSTRAINTS** (USER ENFORCED — обязательно при выполнении этой команды):
> - **Максимум 2 параллельных сабагента одновременно.** Если в фазе больше двух [P]-задач — разбей их на батчи по 2 и запускай батчи последовательно (дождись завершения текущего батча перед стартом следующего). Никогда не запускай в одном сообщении более 2 Agent-вызовов.
> - **Пауза после каждой Phase.** Завершив фазу полностью (все её задачи выполнены и верифицированы), ОСТАНОВИСЬ и дождись явной команды пользователя продолжить. НЕ переходи к следующей Phase автоматически — пользователь сам скажет, когда стартовать следующий этап.
...
4. **PLANNING PHASE** (Execute Before Implementation):
   ...
   - **Atomicity Rule (CRITICAL)**: 1 Task = 1 Agent Invocation
     ...
     * **Parallel execution**: Launch up to **2** agent calls in single message (USER CAP — see EXECUTION CONSTRAINTS). Никогда не превышай 2 параллельных сабагента.
     * Если параллельных задач больше двух — разбей на батчи по 2 и запускай батчи последовательно (дождись завершения текущего батча перед стартом следующего)
     * Example: 3 параллельных задачи → батч A (2 агента в одном сообщении) → ждём → батч B (1 агент)
     * Example: 5 параллельных задач → 3 батча: 2 + 2 + 1, последовательно
...
7. Execute implementation following the task plan:
   ...
   - **Phase-by-phase execution**: Complete each phase before moving to the next. **ОБЯЗАТЕЛЬНО**: после полного завершения текущей Phase (все её задачи выполнены и верифицированы) — ОСТАНОВИСЬ и дождись явной команды пользователя продолжать. Следующую Phase запускает ТОЛЬКО пользователь, не переходи автоматически (см. EXECUTION CONSTRAINTS).
   - **Respect dependencies**: Run sequential tasks in order, parallel tasks [P] can run together — но не более **2 одновременно** (см. EXECUTION CONSTRAINTS); при >2 [P]-задач разбивай на батчи по 2
