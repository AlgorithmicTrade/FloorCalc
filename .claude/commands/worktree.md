...
argument-hint: "<create|remove|list|cleanup> [args]"
allowed-tools: Bash, Read, Write
---

# Git Worktree Manager

Manage git worktrees for parallel feature development. Manual user tool — **not** invoked by subagents (см. Notes).
...
Worktree root directory: `<project>-worktrees/` (sibling of repo). The `<project>` placeholder = `name` field from project root `package.json` (or repo directory name as fallback). Resolve at runtime — **don't hardcode**.
...
- Copies gitignored files per `.worktree-sync.json` (if present)

**Process:**
1. Resolve `<project>` from `package.json#name` (or repo dir name); compose worktree root.
2. Validate feature name (alphanumeric, hyphens only).
3. Check for uncommitted changes (warn if present).
4. Create worktree: `git worktree add ../<project>-worktrees/<name> -b feature/<name> <base>`.
5. If `.worktree-sync.json` exists in repo root — sync files from it:
   - `sync.files`: e.g. `.env`, `.env.local`, `.mcp.json`
   - `sync.directories`: e.g. `.vscode`
   - `sync.patterns`: e.g. `packages/*/.env`, `services/*/.env`
6. Output next steps (open in IDE, commit workflow).
...
**Process:**
1. Find worktree by name (under `<project>-worktrees/`).
...
4. If `--delete-branch`:
   ...
   - Delete remote: `git push origin --delete feature/<name>` (destructive — confirm with user).
...
|------|--------|------|--------|
| main (основной) | main | <repo-root> | Active |
| feature-name | feature/feature-name | ../<project>-worktrees/feature-name | Active |
...
`.worktree-sync.json` is optional (omit if project doesn't need synced gitignored files):
```json
{
  "sync": {
...
If the file is missing — skip step 5 of `create` silently.
...
- **Manual user tool only.** Subagents must NOT create worktrees via `isolation: "worktree"` in Agent tool — параллельные агенты раздували сборку и создавали конфликты. Этот командный wrapper — для интерактивного использования пользователем (см. memory `feedback_no_worktrees.md`).
