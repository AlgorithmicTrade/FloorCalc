...
```bash
# Start review patrol
bd patrol run code-review --var "scope=src/components,topic=refactor"

# Or manually
bd mol wisp codereview --var "scope=src/,topic=audit"
```
...
# Or with scope
bd mol wisp healthcheck --var "scope=packages/web"
```
...
bd mol wisp hotfix --var "issue_description=Login failing,affected_area=auth"
...
```bash
# Start exploration
bd mol wisp exploration --var "question=Should we use Redis for caching?"
...
```bash
# Patch release
bd mol wisp release --var "bump_type=patch"

# Minor release
bd mol wisp release --var "bump_type=minor,message=New feature X"

# Major release
bd mol wisp release --var "bump_type=major,message=Breaking change Y"
```
