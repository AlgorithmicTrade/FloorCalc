---
name: process-logs
description: Process error logs from admin panel - fetch new errors, analyze, create tasks, fix, and mark resolved SKIP: для проектов без admin panel API или без структурированных error logs; для логов без user-impact (debug/trace only); если БД с логами недоступна
---

## Outcome

**Goal**: все новые error логи проанализированы; root cause определён для каждой группы; задачи созданы или ошибки помечены как resolved

**Success criteria**:
- все новые error events с момента last-checkpoint обработаны
- Beads-задачи созданы для action items, группировка по root cause
- обработанные логи помечены resolved в БД (если поддерживается)
...
Beads is the recommended tracker (skip if `.beads/` not initialized). When available, every error should have a Beads task — keeps work tracked.

```bash
# Run this first for each error (when Beads is available):
...
### 3. Context7 lookup (recommended)

Context7 lookup recommended when working with library-specific APIs (skip if MCP unavailable). Query documentation before implementing:
...
Output contract: when updating log status, write a short note. Keep it brief, in English. (This is part of the data-correctness invariant — `log_issue_status` rows without context are unusable downstream.)
...
| `in_progress` | Beads task ID. Example: `Working on <bd-prefix>-5ch`                                                     |
...
1. Edit `<project-package-path>/src/shared/logger/auto-classification.ts`:
...
Before fixing an error, search for similar (closed and open) — likely already solved. Search BOTH sources:
...
4. **Reference in your notes**: `Similar to <bd-prefix>-xxx / <date>. Same fix applied.`

#### 7d. If NOT found — create Beads task (when available)

Every new error should have a Beads task before fixing (when `.beads/` is initialized):
...
- Saved to the `reports/` directory (e.g., `reports/logs-processing-report.md`)
...
> **Important (data-correctness invariant)**: Both DEV and STAGE are production-like servers. Errors on these
...
```
FOR each error:
  1. CREATE BEADS TASK (when available):
...
Save to `reports/logs-processing-report.md` and present to user.

```markdown
## Обработка логов ошибок — Сводка

| Серьёзность | Исправлено | В ожидании | На проверке |
| ----------- | ---------- | ---------- | ----------- |
| CRITICAL    | X          | Y          | Z           |
| ERROR       | X          | Y          | Z           |
| WARNING     | X          | Y          | Z           |

### Созданные задачи Beads:

- <bd-prefix>-xxx: <описание> → <статус>

### В ожидании (требуют уточнения от пользователя):
...
- Error Types: `<project-package-path>/src/shared/logger/types.ts`
- Logs Router: `<project-package-path>/src/server/routers/admin/logs.ts`
