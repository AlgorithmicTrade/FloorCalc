---
name: setup-knip
description: Install and configure Knip for dead code detection. Use before running dead-code-hunter or dependency-auditor to ensure Knip is available. Handles installation, configuration creation, and validation. SKIP: для не-JS/TS проектов (knip — JS-tool); если knip уже установлен и сконфигурирован — пропускать.
