...
description: Execute quality gate validation with configurable blocking behavior. Use when running type-check, build, tests, lint, or custom validation commands in orchestrators or workers to enforce quality standards. SKIP: для проектов без определённого набора quality gates (нет type-check/build/test scripts); для одной команды (используй прямой Bash).
