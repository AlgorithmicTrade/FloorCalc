...
description: Generate properly formatted TodoWrite lists for orchestrator progress tracking. Use when creating phase checklists, updating task status, or ensuring consistent todo formatting. SKIP: для UI-списков задач (это для TodoWrite-tool формата); для постоянного хранения todo (используй issue tracker).
---
