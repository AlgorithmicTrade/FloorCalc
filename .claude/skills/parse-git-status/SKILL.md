...
description: Parse git status output into structured data showing staged, modified, and untracked files. Use for pre-flight validation, checking clean working directory, or listing changed files before commits. SKIP: для нестандартного git output (--porcelain=v2 и др. — нужен отдельный парсер).
---
