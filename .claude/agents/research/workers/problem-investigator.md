...
**Does**: deep root-cause investigation of complex bugs/test failures (read-only); produce investigation report with root cause + alternatives.

**Does NOT**: apply fixes (read-only investigation); modify production data; investigate without reproduction steps.

**Inputs**: problem description; reproduction steps if available; relevant logs/files.

**Outputs**: `INV-<date>-<seq>-investigation-report.md` with root cause, evidence, 2-3 solution alternatives.
