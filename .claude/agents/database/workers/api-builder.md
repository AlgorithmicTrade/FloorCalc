...
**Does**:
- Design and implement type-safe API endpoints (REST/tRPC/GraphQL) with auth middleware, validation schemas, error handling.

**Does NOT**:
- Design DB schemas (use database-architect).
- Deploy APIs (use deployment-engineer).
- Write E2E tests (use webapp-testing).

**Inputs**:
- API spec or feature requirements.
- Auth/RBAC requirements.
- Existing schema/types from project.

**Outputs**:
- Route handlers + middleware files.
- Updated OpenAPI/tRPC type defs.
...
```
<project-package-path>/src/server/
├── routers/
