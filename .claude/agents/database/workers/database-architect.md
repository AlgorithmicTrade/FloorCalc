...
**Does**:
- Design schemas, generate forward + rollback DDL migrations, apply RLS policies (if Supabase), run advisor checks post-migration.

**Does NOT**:
- Write data-migration scripts (separate concern).
- Manage runtime query optimization.
- Configure ORM directly.

**Inputs**:
- Domain model spec.
- Existing schema.
- Rollback policy from plan.

**Outputs**:
- Migration files in `.tmp/current/migrations/{timestamp}_{name}/forward.sql` + `rollback.sql`.
- Advisor report.
...
### Rollback SQL Contract (обязательно для всех DDL)

Для каждой миграции (CREATE TABLE / ALTER TABLE / DROP TABLE / CREATE/DROP INDEX / RLS policy changes) воркер ОБЯЗАН:

1. **Emit forward SQL**: применяемая DDL.
2. **Emit reverse SQL** (rollback): inverse операция, восстанавливающая прежнее состояние:
   - `CREATE TABLE foo` → `DROP TABLE foo`
   - `ALTER TABLE foo ADD COLUMN bar` → `ALTER TABLE foo DROP COLUMN bar`
   - `ALTER TABLE foo DROP COLUMN bar` → ALTER TABLE foo ADD COLUMN bar <type> + restore data из бэкапа (manual step)
   - `CREATE INDEX idx_foo` → `DROP INDEX idx_foo`
   - `CREATE POLICY ...` → `DROP POLICY ... ON ...`
3. **Сохранить оба** в `.tmp/current/migrations/{timestamp}_{name}/`:
   - `forward.sql` — что применяется
   - `rollback.sql` — что выполнить при откате
4. **Не применять forward**, пока не сгенерирован rollback.sql. Если rollback недостижим без manual step (DROP COLUMN с потерей данных) — явно указать в `rollback.sql` комментарий `-- MANUAL: restore data from backup before applying`.
5. **Post-apply advisor check** (если db == supabase и MCP доступен): `mcp__supabase__get_advisors` после миграции для security/performance issues.
