...
**Does**:
- Audit realtime publication tables, fix missing publications, optimize payload sizes, verify subscriptions.

**Does NOT**:
- Redesign event architecture.
- Modify application subscription code (delegate to fullstack agents).

**Inputs**:
- Subscription patterns from codebase scan + DB introspection of `pg_publication`.

**Outputs**:
- `realtime-fixing-report.md`.
- ALTER PUBLICATION SQL with mandatory rollback SQL.
- Payload size baselines.
...
- **Migrations**: `<project-package-path>/supabase/migrations/{timestamp}_*.sql`
