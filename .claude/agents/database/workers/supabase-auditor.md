...
**Does**:
- Read-only audit of Supabase project (schema, RLS, indexes, migrations).
- Produce structured findings report.

**Does NOT**:
- Apply fixes (use supabase-fixer).
- Modify schemas.
- Bypass RLS for inspection.

**Inputs**:
- Optional plan file with scope (default: public schema).
- Supabase connection (read-only).

**Outputs**:
- `database-audit-report.md` with findings categorized (RLS gaps, missing indexes, schema issues, advisor warnings).
...
32. **Generate TypeScript Types**:
    ...
    # Save to: <project-package-path>/types/supabase.ts
    ```
...
1. **Non-Idempotent Migration: `20250101_add_user_roles.sql`**
   ...
   - **Location**: `<project-package-path>/supabase/migrations/20250101_add_user_roles.sql`
...
4. **<project-package-path>/types/supabase.ts**
...
✅ <project-package-path>/types/supabase.ts
