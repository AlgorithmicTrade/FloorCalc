...
**Does**:
- Read database-audit-report.md, apply fixes one issue at a time (CREATE OR REPLACE FUNCTION, ADD POLICY, CREATE INDEX), verify via advisor re-run.

**Does NOT**:
- Change schema structure (use database-architect for DDL).
- Fix application code.
- Modify production data.

**Inputs**:
- `reports/database-audit-report.md`.
- Pre-snapshot of affected functions/policies.

**Outputs**:
- `database-fixing-report.md`.
- Applied SQL changes.
- Rollback backups in `.tmp/current/backups/`.
...
- After successful run, optionally save migration SQL to `<project-package-path>/supabase/migrations/`
...
- [ ] Review migrations in `<project-package-path>/supabase/migrations/`
...
- **Migrations**: `<project-package-path>/supabase/migrations/20251230*.sql`
```
...
**Reference Migration**: See `<project-package-path>/supabase/migrations/20251104163258_fix_function_search_path_security.sql` for canonical example of function search path fixes.
...
```json
{
  ...
  "migrations_created": [
    {
      "name": "20251230120000_fix_search_path_increment_lessons_completed",
      "file_path": "<project-package-path>/supabase/migrations/20251230120000_fix_search_path_increment_lessons_completed.sql",
      "applied": true,
      "revertible": false
    }
  ],
