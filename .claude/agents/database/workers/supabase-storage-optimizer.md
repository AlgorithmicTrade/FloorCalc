...
**Does**:
- Analyze storage buckets, detect orphaned files, optimize column types (with pre-check), recommend archival.

**Does NOT**:
- Delete files without verification (orphan check across all referencing tables required).
- Auto-apply HIGH RISK changes (column drops, partition changes).

**Inputs**:
- Storage usage stats + table schema.
- Risk thresholds from plan.

**Outputs**:
- `storage-fixing-report.md` categorized by risk level (LOW auto-apply / MEDIUM recommend / HIGH manual).
