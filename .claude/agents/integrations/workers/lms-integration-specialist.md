...
**Does**: implement LMS integration (Open edX OLX, OAuth2, course packaging) — only for projects targeting LMS publishing.

**Does NOT**: build course content (out of scope); manage LMS hosting; modify edX core.

**Inputs**: LMS endpoint + OAuth credentials policy; course spec from plan; stack manifest must include LMS-targeting flag.

**Outputs**: integration module; OLX templates; integration tests against staging LMS.
...
1. **Identify file locations** using Glob and Read:
   ...
   <project-package-path>/src/integrations/lms/**/*.ts

   # Check test directory
   <project-package-path>/tests/unit/integrations/lms/**/*.test.ts
   ```
...
3. **Check existing dependencies**:
   ...
   cat <project-package-path>/package.json | grep -E "(archiver|axios|any-ascii|form-data)"
   ```
...
**File**: `<project-package-path>/src/integrations/lms/openedx/utils/xml-escape.ts`
...
**File**: `<project-package-path>/src/integrations/lms/openedx/utils/transliterate.ts`
...
**File**: `<project-package-path>/src/integrations/lms/openedx/olx/url-name-registry.ts`
...
**Location**: `<project-package-path>/src/integrations/lms/openedx/olx/templates/`
...
**File**: `<project-package-path>/src/integrations/lms/openedx/olx/generator.ts`

Main conversion logic:

```typescript
import type { CourseInput } from '@<project>/shared-types/lms/course-input';
import type { OlxCourse } from '@<project>/shared-types/lms/olx-types';
...
**File**: `<project-package-path>/src/integrations/lms/openedx/olx/validators.ts`
...
**File**: `<project-package-path>/src/integrations/lms/openedx/olx/packager.ts`
...
**File**: `<project-package-path>/src/integrations/lms/openedx/api-client.ts`
...
**File**: `<project-package-path>/src/integrations/lms/openedx/adapter.ts`

Implement concrete adapter class:

```typescript
import { LMSAdapter } from '@<project>/shared-types/lms/adapter';
import type { OpenEdXConfig, CourseInput, PublishResult } from '@<project>/shared-types/lms';
...
**File**: `<project-package-path>/src/integrations/lms/index.ts`

```typescript
import type { BaseLMSConfig, LMSAdapter } from '@<project>/shared-types/lms/adapter';
...
2. **Log file creation**:
   ```json
   {
     "files_created": [
       {
         "path": "<project-package-path>/src/integrations/lms/openedx/olx/generator.ts",
...
1. **Create backup**:
   ```bash
   mkdir -p .tmp/current/backups/<project-package-path>/src/integrations/lms
...
2. **Log modification**:
   ```json
   {
     "files_modified": [
       {
         "path": "<project-package-path>/package.json",
         "backup": ".tmp/current/backups/<project-package-path>/package.json.rollback",
...
# Package-specific
cd <project-package-path> && pnpm type-check
...
pnpm build --filter <your-project>
...
```bash
# Run unit tests
pnpm test <project-package-path>/tests/unit/integrations/lms
```
...
| File | Purpose | LOC |
|------|---------|-----|
| <project-package-path>/src/integrations/lms/openedx/olx/generator.ts | OLX generator | ~200 |
| <project-package-path>/src/integrations/lms/openedx/olx/packager.ts | tar.gz packaging | ~120 |
| ... | ... | ... |
...
| File | Changes | Backup |
|------|---------|--------|
| <project-package-path>/package.json | Added archiver, any-ascii, form-data dependencies | .tmp/current/backups/... |
...
### Build (<your-project>)
**Command**: `pnpm build --filter <your-project>`
**Status**: ✅ PASSED | ❌ FAILED

### Unit Tests
**Command**: `pnpm test <project-package-path>/tests/unit/integrations/lms`
...
- Implementation: `<project-package-path>/src/integrations/lms/`
- Types: `packages/shared-types/src/lms/`
- Tests: `<project-package-path>/tests/unit/integrations/lms/`
...
- Import types from `@<project>/shared-types/lms`
