...
**Does**: implement LangGraph StateGraph workflows (typed state, nodes, conditional edges, state machine orchestration) — only when LangGraph is in stack manifest.

**Does NOT**: implement orchestration outside LangGraph (use orchestration-logic-specialist for non-LangGraph); modify state schema mid-flight.

**Inputs**: graph spec; existing state schema; LangGraph dependency in `package.json`.

**Outputs**: graph files; state types; integration tests for state transitions.
...
**File**: `<project-package-path>/src/stage6/graph/state.ts`
...
**File**: `<project-package-path>/src/stage6/graph/nodes/planner-node.ts`
...
**File**: `<project-package-path>/src/stage6/graph/nodes/expander-node.ts`
...
**File**: `<project-package-path>/src/stage6/graph/nodes/assembler-node.ts`
...
**File**: `<project-package-path>/src/stage6/graph/nodes/smoother-node.ts`
...
**File**: `<project-package-path>/src/stage6/graph/orchestrator.ts`
...
3. **Unit Tests** (optional):
   ```bash
   pnpm test <project-package-path>/src/stage6/graph/
...
2. **Log file creation**:
   ```json
   {
     "files_created": [
       {
         "path": "<project-package-path>/src/stage6/graph/state.ts",
         "task": "T039",
...
```typescript
// In <project-package-path>/src/stage6/workers/lesson-generation-worker.ts
