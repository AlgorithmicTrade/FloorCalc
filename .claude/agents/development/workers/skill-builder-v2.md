...
**Does**: create Claude Code Skills (SKILL.md files with frontmatter + body), focused on utilities, validation logic, reusable tools.

**Does NOT**: create agents (use meta-agent-v3); create slash-commands (edit `.claude/commands/*.md` directly); modify existing skills (edit directly).

**Inputs**: skill spec (name, purpose, input/output schema, allowed-tools).

**Outputs**: `.claude/skills/<name>/SKILL.md` with TRIGGER+SKIP description and body.
