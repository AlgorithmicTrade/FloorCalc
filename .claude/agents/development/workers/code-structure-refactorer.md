...
**Does**: refactor code structure (move files, consolidate scattered modules, update imports) preserving behaviour and git history.

**Does NOT**: change runtime semantics; rewrite business logic; restructure on first batch without validation.

**Inputs**: refactor plan with target structure; current codebase scope.

**Outputs**: moved/renamed files; updated imports across codebase; per-batch validation log.
      ...
      "batches": [
        {
          ...
          "files_moved": [
            {
              "from": "<project-package-path>/src/stage2/orchestrator.ts",
              "to": "<project-package-path>/src/stages/stage2-planning/orchestrator.ts",
...
24. **Report structure**:
    ...
    **Duration**: {duration}
    ...
    |--------------|----------|
    | `<project-package-path>/src/stage2/orchestrator.ts` | `<project-package-path>/src/stages/stage2-planning/orchestrator.ts` |
    | ... | ... |

    ### Files Modified ({count})

    Files with updated imports:
    - `<project-package-path>/src/index.ts`
    - `<project-package-path>/src/handlers/handlerRegistry.ts`
    - ...
...
```json
{
  ...
  "config": {
    "currentStructure": {
      "stage2": [
        "<project-package-path>/src/stage2/orchestrator.ts",
        "<project-package-path>/src/stage2/handler.ts",
        "<project-package-path>/src/stage2/phases/*.ts",
        "<project-package-path>/src/stage2/validators/*.ts"
      ],
      "stage3": [
        "<project-package-path>/src/stage3/orchestrator.ts",
        "<project-package-path>/src/stage3/handler.ts",
        "<project-package-path>/src/stage3/phases/*.ts"
      ],
      "stage4": [
        "<project-package-path>/src/stage4/*.ts"
      ],
      "stage5": [
        "<project-package-path>/src/stage5/*.ts"
      ]
    },
    "targetStructure": {
      "pattern": "<project-package-path>/src/stages/stage{N}-{name}/",
