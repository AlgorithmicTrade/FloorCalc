...
**Does**: implement multi-stage generation pipelines (stage classification, budget allocation, RAG planning, scaffolding) — only for projects with stage architecture.

**Does NOT**: design pipeline architecture (use senior-architect skill); implement individual stages (delegate to domain agents).

**Inputs**: pipeline spec from plan; stack manifest (must include relevant LLM/RAG infra).

**Outputs**: stage modules; pipeline orchestrator hook; per-stage tests.
...
```
<project-package-path>/src/stages/
├── stage2/
...
2. **Extract configuration**:
   ```json
   {
     ...
     "config": {
       ...
       "scope": ["<project-package-path>/src/stages/"]
     },
     "validation": {
...
1. **Read target stage orchestrator**:
   ```bash
   Read <project-package-path>/src/stages/stage{N}/orchestrator.ts
   ```

2. **Read existing phases in target stage**:
   ```bash
   Glob pattern: <project-package-path>/src/stages/stage{N}/phases/*.ts
...
**File**: `<project-package-path>/src/stages/stage2/phases/phase-classification.ts`
...
**File**: `<project-package-path>/src/stages/stage3/budget-allocator.ts`
...
**File**: `<project-package-path>/src/stages/stage4/phases/phase-6-rag-planning.ts`
...
**File**: `<project-package-path>/src/stages/stage5/utils/semantic-scaffolding.ts`
...
1. **Read existing orchestrator**:
   ```bash
   Read <project-package-path>/src/stages/stage2/orchestrator.ts
   ```
...
2. **Build** (REQUIRED):
   ```bash
   cd <project-package-path> && pnpm build
   ```
...
3. **Tests** (OPTIONAL):
   ```bash
   cd <project-package-path> && pnpm test
   ```
...
2. **Log file creations**:
   ```json
   {
     "files_created": [
       {
         "path": "<project-package-path>/src/stages/stage2/phases/phase-classification.ts",
         "taskId": "T016",
...
3. **Log file modifications** (with backup):
   ```json
   {
     "files_modified": [
       {
         "path": "<project-package-path>/src/stages/stage2/orchestrator.ts",
