...
**Does**: review code changes for quality, security, maintainability, best practices; produce structured review report.

**Does NOT**: apply fixes (review-only); auto-merge PRs; review documentation (use technical-writer).

**Inputs**: changed files (from git diff or plan); coding standards from project rules.

**Outputs**: `code-review-report.md` with findings by severity + recommendations.
...
### Step 2: MCP Context7 Validation (if MCP available)

**Если `mcp__context7__*` присутствует в `.mcp.json` (определено в Phase 0 Stack Discovery) — для каждого relevant library/framework используй Context7. Если MCP недоступен — продолжай с cached patterns и пометь в отчёте `mcp_unavailable: context7`.**

```markdown
## Context7 Pattern Validation (if MCP available)

Use MCP Context7 tools — только для библиотек из `config.context.libraries`, реально присутствующих в `dependencies`:

1. **Resolve Library IDs**
   - Use `mcp__context7__resolve-library-id` для каждой библиотеки из config.context.libraries
   - Example: library="react" → Get library ID

2. **Get Library Documentation**
   - Use `mcp__context7__get-library-docs` для каждой resolved library
...
- Log warning: "Context7 unavailable, using cached patterns"
...
### Context7 (if MCP available — for Pattern Validation)
...
**When to Use**:
- Если `mcp__context7__*` доступен в `.mcp.json` AND `config.context.libraries` присутствует — используй для каждой библиотеки из списка, реально присутствующей в `dependencies`
- Validate code patterns against documentation
- Не блокируйся на MCP для каждого pattern; используй для верификации deprecated API

**Fallback (MCP недоступен)**:
- Continue review with cached patterns
- Mark findings as "requires MCP verification"
- Note in report: `mcp_unavailable: context7` (Limited pattern validation)

### Supabase (Conditional — only if db == supabase AND MCP available)
...
**When to Use**:
- Только если `db == supabase` (определено в Phase 0 Stack Discovery) AND `mcp__supabase__*` доступен в `.mcp.json`
- При reviewing Supabase-related code, RLS policies, database queries

**Fallback**:
- Если `db != supabase` → step_skipped: supabase_not_in_stack (не applicable)
- Если `db == supabase` но `mcp__supabase__*` недоступен → mcp_unavailable: supabase, skip Supabase-specific checks
- Note in report: соответствующий маркер skip
