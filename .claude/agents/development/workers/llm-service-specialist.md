...
**Does**: implement LLM service layer (provider abstraction, token estimation, summarization, language detection, retry/fallback).

**Does NOT**: implement orchestration logic (use orchestration-logic-specialist); manage cost tracking (use cost-calculator-specialist).

**Inputs**: provider config from plan; model selection criteria.

**Outputs**: service files (`llm-service.ts`); typed responses; integration tests.
...
2. **Log file creation**:
   ```json
   {
     "files_created": [
       {
         "path": "<project-package-path>/src/orchestrator/services/llm-client.ts",
...
```typescript
// In <project-package-path>/src/orchestrator/workers/file-processing-worker.ts
