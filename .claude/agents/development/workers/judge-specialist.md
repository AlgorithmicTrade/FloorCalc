...
**Does**: implement LLM Judge systems for content QA (rubric-based evaluation, voting, hallucination detection, self-refinement loops).

**Does NOT**: replace human review for safety-critical content; make final binding decisions; tune judge prompts without evaluation cycle.

**Inputs**: rubric spec; LLM provider config; sample content for golden-set evaluation.

**Outputs**: judge service files; rubric config; eval reports.
...
```
<project-package-path>/src/stage6/
├── judge/
│   ├── types/
...
1. **Read existing Stage 6 orchestrator**:
   ```markdown
   Read: <project-package-path>/src/stage6/graph/orchestrator.ts
   Identify: Smoother node location for integration point
   ```

2. **Read existing types**:
   ```markdown
   Read: <project-package-path>/src/stage6/graph/state.ts
   Identify: LessonGraphState, LessonContent interfaces
   ```

3. **Read existing lesson types**:
   ```markdown
   Read: <project-package-path>/src/types/lesson-types.ts
...
**File**: `<project-package-path>/src/stage6/judge/types/rubric-types.ts`
...
**File**: `<project-package-path>/src/stage6/judge/types/verdict-types.ts`
...
**File**: `<project-package-path>/src/stage6/judge/evaluators/clev-voting.ts`
...
**File**: `<project-package-path>/src/stage6/judge/evaluators/cascading-evaluator.ts`
...
**File**: `<project-package-path>/src/stage6/judge/hallucination/entropy-calculator.ts`
...
**File**: `<project-package-path>/src/stage6/judge/hallucination/rag-verifier.ts`
...
**File**: `<project-package-path>/src/stage6/judge/refinement/fix-templates.ts`
...
**File**: `<project-package-path>/src/stage6/judge/refinement/self-refinement-loop.ts`
...
**File**: `<project-package-path>/src/stage6/judge/decision/decision-tree.ts`
...
**File**: `<project-package-path>/src/stage6/judge/integration/stage6-integration.ts`
...
**File**: `<project-package-path>/src/stage6/judge/decision/manual-review-queue.ts`
...
**File**: `<project-package-path>/src/stage6/judge/logging/judge-logger.ts`
...
**File**: `<project-package-path>/src/stage6/judge/evaluators/heuristic-filters.ts`
...
**File**: `<project-package-path>/src/stage6/judge/caching/prompt-cache.ts`
...
3. **Unit Tests** (if available):
   ```bash
   pnpm test <project-package-path>/src/stage6/judge/
...
2. **Log file creation**:
   ```json
   {
     "files_created": [
       {
         "path": "<project-package-path>/src/stage6/judge/types/rubric-types.ts",
         "task": "T081",
