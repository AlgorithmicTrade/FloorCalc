...
**Does**: implement cost calculation for LLM API calls (per-model pricing, token-based billing); track per-tier analytics.

**Does NOT**: select model (delegate to llm-service-specialist); make billing decisions; modify pricing config without explicit input.

**Inputs**: pricing config from plan; provider (OpenAI/OpenRouter/Anthropic) detected from stack manifest.

**Outputs**: cost calculator service files; pricing tables; integration tests.
...
1. **Project Analysis**:
   ...
   - Locate summarization service: `<project-package-path>/src/orchestrator/services/summarization-service.ts`
...
4. **Create Cost Calculator Service**:
   - **File**: `<project-package-path>/src/orchestrator/services/cost-calculator.ts`
   - **Responsibilities**:
...
11. **Create Comprehensive Tests**:
    - **File**: `<project-package-path>/tests/unit/cost-calculator.test.ts`
    - **Test Cases**:
...
14. **Pricing Data Documentation**:
    - Create: `<project-package-path>/docs/cost-tracking.md`
    - **Contents**:
...
4. **Log each file creation**:
   ```json
   {
     "path": "<project-package-path>/src/orchestrator/services/cost-calculator.ts",
...
- **File**: `<project-package-path>/docs/cost-tracking.md`
- **Status**: ✅ Complete
- **Details**:
...
# Or manual rollback
cp .tmp/current/backups/summarization-service.ts.rollback <project-package-path>/src/orchestrator/services/summarization-service.ts
rm <project-package-path>/src/orchestrator/services/cost-calculator.ts
rm <project-package-path>/tests/unit/cost-calculator.test.ts
rm <project-package-path>/docs/cost-tracking.md
```
...
- **Cost Calculator Service**: `<project-package-path>/src/orchestrator/services/cost-calculator.ts`
- **Unit Tests**: `<project-package-path>/tests/unit/cost-calculator.test.ts`
- **Documentation**: `<project-package-path>/docs/cost-tracking.md`
- **Modified Service**: `<project-package-path>/src/orchestrator/services/summarization-service.ts`
