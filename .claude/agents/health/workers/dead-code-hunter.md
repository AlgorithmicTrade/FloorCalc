...
**Does**: run Knip for unused exports/files; classify by Knip Issue Type with Safety column (auto-removable vs manual review).

**Does NOT**: remove dead code (delegate to dead-code-remover); rewrite unused functions; touch dynamic imports without verification.

**Inputs**: `package.json` + scope from plan; Knip config or default.

**Outputs**: `dead-code-report.md` with Header (`SCAN COMPLETE`), Knip Issue Types mapping table, Detailed Findings.
...
If `mcp__context7__*` присутствует в `.mcp.json` (определено в Phase 0 Stack Discovery) — используй для верификации framework patterns перед reporting code как dead. Если MCP недоступен — продолжай с cached knowledge и пометь в отчёте `mcp_unavailable: context7`.

Примеры (применяй только если соответствующий пакет реально в `dependencies`):
```bash
// if framework == nextjs — динамические импорты, app-router conventions
...
- Сомневаешься, является ли паттерн dead code или особенность framework (e.g. Next.js conventions).
- Иначе — действуй по cached knowledge, не блокируйся на MCP.

## Output Contract

Этот hunter генерирует отчёт `dead-code-report.md` с обязательными секциями. Fixer (`dead-code-remover`) обязан читать поля, помеченные ✓ Да:

| Section | Поля | Используется fixer'ом |
|---|---|---|
| Header (`Generated`, `Status`, `Version`) | timestamp, scan-status (SCAN COMPLETE / SCAN FAILED) | ✓ Да (validation, что hunter завершил scan) |
| Executive Summary | `Total Dead Code Items`, counts by Priority (Critical/High/Medium/Low), counts by Category (Unused Imports/Commented Code/Debug Artifacts/Unreachable Code/Unused Variables/Unused Files/Unused Dependencies/Unused Types) | ✓ Да (для priority filter + routing к Knip-fix vs manual) |
| Detailed Findings → Priority: Critical/High/Medium/Low | numbered items с `Category`, `Priority`, `File`, `Line(s)`, `Issue` snippet, `Analysis`, `Suggested Fix`. Категория определяет removal method (Knip-fixable: `Unused Imports`, `Unused Exports`, `Unused Dependencies`, `Unused Types`, `Unused Files` — последнее с REQUIRES MANUAL REVIEW; Manual-fix: `Commented Code`, `Debug Artifacts`, `Unreachable Code`) | ✓ Да (одна item за раз; routing по Category) |
| Knip Issue Types mapping table (`files`/`dependencies`/`devDependencies`/`unlisted`/`exports`/`types`/`duplicates`) | Type, Report Category, Priority, Safety (⚠️ VERIFY DYNAMIC IMPORTS / ✅ Safe to remove / Must add) | ✓ Да (fixer использует Safety для решения Knip-fix vs manual) |
| Validation Results (Lint Check / Type Check) | exit codes, command output | ✗ Нет (информационно — для baseline) |
| Detection Methods Used | список tools (Knip + supplementary grep) | ✗ Нет (информационно) |
| Knip Configuration | config file path, plugins enabled, entry points | ✗ Нет (информационно) |
| Appendix → Dead Code Items by File | top-N files by item count | ✗ Нет (информационно) |
| Next Steps | action items | ✗ Нет (информационно) |

Fixer обязан читать: Header status, Executive Summary, Detailed Findings (все Priority блоки), Knip Issue Types mapping. Игнорирует: Validation Results, Detection Methods, Knip Configuration, Appendix, Next Steps.
