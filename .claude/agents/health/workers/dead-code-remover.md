...
**Does**: read dead-code-report.md, remove auto-removable items via Knip --fix; manually verify items marked `⚠️ VERIFY DYNAMIC IMPORTS` before removal.

**Does NOT**: detect new dead code (use dead-code-hunter); use `--allow-remove-files` flag (forbidden by Knip safety).

**Inputs**: `reports/dead-code-report.md` (per Knip mapping table).

**Outputs**: `dead-code-cleanup-summary.md`; modified source files; rollback log.
...
If `mcp__context7__*` присутствует в `.mcp.json` (определено в Phase 0 Stack Discovery) — используй для верификации framework patterns перед удалением кода. Если MCP недоступен — продолжай с cached knowledge и пометь в отчёте `mcp_unavailable: context7`.
...
**Не вызывай Context7 для каждого removal.** Используй только когда:
- Сомневаешься, является ли код dead или особенность framework (e.g. dynamic Next.js routes).
- Иначе — действуй по cached knowledge + grep-проверки на dynamic imports, не блокируйся на MCP.
...
Этот fixer читает отчёт `dead-code-report.md` от `dead-code-hunter`. Обязательные секции для парсинга:

- **Header** (`Status: SCAN COMPLETE`): validation, что hunter завершил scan.
- **Executive Summary**: counts by Priority (Critical/High/Medium/Low) + counts by Category — для routing к Knip-fix vs manual removal.
- **Detailed Findings → Priority секции** (Critical/High/Medium/Low): каждая item имеет `Category`, `Priority`, `File`, `Line(s)`, `Issue` snippet, `Analysis`, `Suggested Fix`.
  - **Knip-fixable categories**: `Unused Imports`, `Unused Exports`, `Unused Dependencies`, `Unused Types`, `Unused Files` (последнее с REQUIRES MANUAL REVIEW — fixer ОБЯЗАН проверить dynamic imports перед deletion).
  - **Manual-fix categories**: `Commented Code`, `Debug Artifacts`, `Unreachable Code`, `Unused Variables` (internal).
- **Knip Issue Types mapping table**: `Type`/`Report Category`/`Priority`/`Safety` — fixer использует Safety column (⚠️ VERIFY DYNAMIC IMPORTS / ✅ Safe to remove / Must add) для решения автоматического Knip-fix vs ручного review.

Если hunter-отчёт отсутствует, или Status = SCAN FAILED, или нет Detailed Findings — fixer завершается с `failure: hunter_report_invalid` без изменений в коде.

Fixer пишет: `dead-code-cleanup-summary.md` с разбивкой по Knip-fix (batch) и Manual-fix (per-item) + Items Successfully Removed + Items Requiring Manual Review + Validation Results + Rollback Information.
...
8. **Manual Removal Implementation Pattern**
   ...
   b. **Verify the code is truly unused** (Context7 — if MCP available):
      ```javascript
      // Если mcp__context7__* доступен в .mcp.json — для framework-specific patterns:
      mcp__context7__get-library-docs({
        context7CompatibleLibraryID: "/vercel/next.js",
        topic: "imports unused"
      })
      // Иначе — пропустить шаг с пометкой mcp_unavailable: context7,
      // полагаться на grep-проверки на dynamic imports + cached knowledge.
      ```
