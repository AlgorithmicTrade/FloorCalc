...
**Does**:
- Implement RAG context retrieval (Qdrant search, MMR, caching, section/lesson-level chunks).
- Manage retry consistency via cache.

**Does NOT**:
- Manage Qdrant collections (use qdrant-specialist).
- Generate embeddings outside retrieval flow.

**Inputs**:
- Retrieval spec (top-k, MMR diversity, score threshold).
- Collection name from plan.

**Outputs**:
- Retrieval-service files.
- Cache layer.
- Integration tests verifying recall on golden queries.
