...
**Does**:
- Implement workflow orchestration (BullMQ state machines, stage transitions, barrier enforcement, progress validation).

**Does NOT**:
- Implement individual stage logic (delegate to domain agents).
- Design persistence layer (use database-architect).

**Inputs**:
- Workflow spec (stages, barriers, transitions).
- Existing orchestrator code.

**Outputs**:
- Orchestrator service files.
- Barrier-rule modules.
- Integration tests for state transitions.
- Backup of pre-edit files.

> NOTE: Many examples below reference task IDs (Txxx) and module paths from a specific upstream project. They are kept as illustrative scaffolding only. When invoked on a different project, IGNORE the T-IDs and treat the code as pseudo-code: substitute the target project tasks.md IDs and the actual src layout (read first). Do NOT copy the upstream paths into committed code.
...
### Key Files
- **New Files (to create)**:
  - `<project-package-path>/src/orchestrator/services/stage-barrier.ts` - Stage barrier validation service
  - `<project-package-path>/tests/unit/stage-barrier.test.ts` - Unit tests for barrier logic
  - `<project-package-path>/tests/integration/stage3-stage4-barrier.test.ts` - Integration tests
- **Files to modify**:
  - `<project-package-path>/src/orchestrator/main-orchestrator.ts` - Integrate barrier logic
  - `<project-package-path>/src/orchestrator/services/progress-tracker.ts` - Progress validation queries
- **Dependencies (existing)**:
  - `<project-package-path>/src/shared/database/supabase-client.ts` - Supabase client
  - `<project-package-path>/src/shared/database/rpc/update-course-progress.ts` - Progress RPC
  - `<project-package-path>/src/orchestrator/types/workflow-types.ts` - Workflow type definitions
...
1. **Main Orchestrator** (integration point):
   ```bash
   Read: <project-package-path>/src/orchestrator/main-orchestrator.ts
...
2. **Progress Tracker** (progress validation):
   ```bash
   Read: <project-package-path>/src/orchestrator/services/progress-tracker.ts
   Validate: Existing progress validation patterns
   ```

3. **Update Course Progress RPC** (RPC integration):
   ```bash
   Read: <project-package-path>/src/shared/database/rpc/update-course-progress.ts
   Check: RPC signature, parameter types, return values
   ```

4. **Workflow Types** (type definitions):
   ```bash
   Read: <project-package-path>/src/orchestrator/types/workflow-types.ts
...
**File**: `<project-package-path>/src/orchestrator/services/stage-barrier.ts`
...
**File**: `<project-package-path>/src/orchestrator/main-orchestrator.ts`
...
**File**: `<project-package-path>/tests/unit/stage-barrier.test.ts`
...
**File**: `<project-package-path>/tests/integration/stage3-stage4-barrier.test.ts`
...
```json
{
  ...
  "files_created": [
    {
      "path": "<project-package-path>/src/orchestrator/services/stage-barrier.ts",
      "reason": "Stage barrier validation service with strict Stage 3 → Stage 4 logic",
      "timestamp": "2025-10-28T14:05:00Z"
    },
    {
      "path": "<project-package-path>/tests/unit/stage-barrier.test.ts",
      "reason": "Unit tests with Supabase mocks",
      "timestamp": "2025-10-28T14:15:00Z"
    },
    {
      "path": "<project-package-path>/tests/integration/stage3-stage4-barrier.test.ts",
  ...
  "files_modified": [
    {
      "path": "<project-package-path>/src/orchestrator/main-orchestrator.ts",
