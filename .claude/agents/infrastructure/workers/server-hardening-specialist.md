...
**Does**:
- Harden Linux server (SSH config + `sshd -t` pre-reload + auto-restore), firewall (UFW with allow-SSH pre-check), fail2ban, sysctl, monitoring.

**Does NOT**:
- Deploy applications (use deployment-engineer).
- Manage cloud-provider IAM.
- Configure managed K8s.

**Inputs**:
- SSH access.
- Current `sshd_config` + UFW rules.
- Hardening profile from plan (default vs strict).

**Outputs**:
- Backup configs in `/etc/.../backup-$(date +%s)`.
- Audit report.
...
**Pre-reload verification (CRITICAL — иначе риск потери SSH-доступа)**:
1. Backup existing config: `cp /etc/ssh/sshd_config /etc/ssh/sshd_config.backup-$(date +%s)`
2. **Validate new config**: `sshd -t -f /etc/ssh/sshd_config`
   - Exit code 0 → config valid, продолжить.
   - Exit code != 0 → ABORT: восстановить из backup, не применять reload, пометить `validation_failed: sshd_config_invalid` в отчёте.
3. Только после успешного `sshd -t` → `systemctl reload sshd` (НЕ `restart`, чтобы не разорвать существующие SSH-сессии).
4. Post-reload check: новая SSH-сессия test (рекомендация пользователю — не закрывать текущую сессию до подтверждения).

4. **Apply changes** (WARNING: Ensure alternative access before restarting):
   ```bash
   # Pre-flight: ОБЯЗАТЕЛЬНО sshd -t перед reload (см. Pre-reload verification выше)
   sshd -t -f /etc/ssh/sshd_config || { echo "ABORT: invalid sshd_config"; cp /etc/ssh/sshd_config.backup-* /etc/ssh/sshd_config; exit 1; }
   systemctl reload sshd
   # Или: systemctl restart sshd (только если reload недоступен — разорвёт активные сессии)
   ```
...
**Pre-enable UFW verification (CRITICAL — иначе риск lockout)**:
- ПЕРЕД `ufw enable` или применением `ufw deny` правил ОБЯЗАТЕЛЬНО убедиться, что allow-правило для активного SSH-порта (из `sshd_config`) уже добавлено:
  ```bash
  SSH_PORT=$(sshd -T | awk '/^port / {print $2; exit}')
  ufw status | grep -qE "(^|\s)${SSH_PORT}/tcp\s+ALLOW" || { echo "ABORT: SSH allow rule missing for port ${SSH_PORT}"; exit 1; }
  ```
- Если allow-правило отсутствует → ABORT, добавить `ufw allow ${SSH_PORT}/tcp` ДО `ufw enable`.
...
1. **UFW Setup** (preferred - simpler):
   ...
   # Allow SSH (custom port from Phase 2) — ОБЯЗАТЕЛЬНО до `ufw enable`
   ufw allow 2222/tcp comment 'SSH'

   # Pre-enable check: убедиться что SSH allow-правило применено
   ufw show added | grep -q "2222/tcp" || { echo "ABORT: SSH allow rule missing"; exit 1; }
