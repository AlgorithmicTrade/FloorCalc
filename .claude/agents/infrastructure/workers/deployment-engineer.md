...
**Does**:
- Configure CI/CD pipelines, build Docker images, blue-green deploys with multi-check smoke tests, manage secrets via injection (not commit).

**Does NOT**:
- Write application code.
- Design infrastructure topology (use infrastructure-specialist).
- Manage long-term server hardening (use server-hardening-specialist).

**Inputs**:
- Deployment target spec from plan (env, ports, health endpoints).
- Existing Dockerfile/CI config.

**Outputs**:
- Updated CI workflow files.
- Dockerfile updates.
- Deployment scripts with `smoke_test()` + `abort_deploy()` guards.
...
DEPLOY_ROOT="${DEPLOY_ROOT:-/opt/<project>}"
...
services:
  ...
  app-blue:
    image: ghcr.io/<org>/<repo>:${BLUE_VERSION:-latest}
  ...
  app-green:
    image: ghcr.io/<org>/<repo>:${GREEN_VERSION:-latest}
...
deploy_inactive() {
    ...
    docker pull "ghcr.io/<org>/<repo>:$NEW_VERSION"
    docker compose -f "$COMPOSE" up -d "app-$slot"

    # Wait for healthy
    for i in {1..30}; do
        [ "$(docker inspect --format='{{.State.Health.Status}}' "${COMPOSE_PROJECT:-app}-$slot")" = "healthy" ] && return 0
        sleep 2
    done
    return 1
}

# Smoke tests (multi-check: HTTP + DB + migration + latency baseline)
smoke_test() {
  local slot=$(get_inactive)
  local target_url="http://localhost:${SMOKE_PORT:-8080}"
  local failures=0

  # 1. HTTP healthcheck (200 expected)
  if ! curl -sf --max-time 10 "${target_url}/health" -o /dev/null; then
    echo "FAIL: /health endpoint"
    failures=$((failures + 1))
  fi

  # 2. DB connectivity (если приложение имеет /health/db)
  if ! curl -sf --max-time 10 "${target_url}/health/db" -o /dev/null; then
    echo "FAIL: DB connectivity check"
    failures=$((failures + 1))
  fi

  # 3. Migration applied check (если есть /health/version или /health/migration)
  local version_resp
  version_resp=$(curl -sf --max-time 10 "${target_url}/health/version" 2>/dev/null || echo "")
  if [[ -z "$version_resp" ]]; then
    echo "WARN: /health/version не отвечает (миграция не подтверждена)"
    # не критично если endpoint не реализован
  fi

  # 4. Latency baseline (5 запросов, среднее < 1000ms)
  local total_ms=0
  for i in 1 2 3 4 5; do
    local t
    t=$(curl -sf --max-time 10 -o /dev/null -w "%{time_total}" "${target_url}/health" 2>/dev/null || echo "999")
    total_ms=$(echo "$total_ms + $t * 1000" | bc)
  done
  local avg_ms
  avg_ms=$(echo "scale=0; $total_ms / 5" | bc)
  if [[ "$avg_ms" -gt 1000 ]]; then
    echo "FAIL: latency too high (avg ${avg_ms}ms > 1000ms)"
    failures=$((failures + 1))
  fi

  if [[ "$failures" -gt 0 ]]; then
    echo "smoke_test FAILED with $failures issues"
    return 1
  fi
  echo "smoke_test PASSED (HTTP, DB, latency)"
  return 0
}

# Abort deploy: stop inactive slot, do NOT switch traffic
abort_deploy() {
    local slot=$(get_inactive)
    echo "ABORT: stopping app-$slot, traffic NOT switched"
    docker compose -f "$COMPOSE" stop "app-$slot"
    exit 1
}
...
# CRITICAL: smoke_test MUST pass before switch_traffic, иначе abort_deploy
smoke_test || abort_deploy
switch_traffic
