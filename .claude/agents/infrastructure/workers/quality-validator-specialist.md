...
**Does**:
- Implement semantic similarity validation (Jina-v3 embeddings + cosine), quality gates, retry-on-low-similarity logic.
...
### Key Files
- **New Files (to create)**:
  - `<project-package-path>/src/orchestrator/services/quality-validator.ts` - Quality validation service
  - `<project-package-path>/tests/unit/quality-validator.test.ts` - Unit tests with mocks
- **Files to modify**:
  - `<project-package-path>/src/orchestrator/services/summarization-service.ts` - Integrate quality gate
  - `<project-package-path>/src/orchestrator/workers/stage-3-create-summary-worker.ts` - Retry logic integration
- **Dependencies (existing)**:
  - `<project-package-path>/src/shared/integrations/qdrant/client.ts` - Qdrant client
  - `<project-package-path>/src/shared/embeddings/generate.ts` - Jina-v3 embedding generation
  - `<project-package-path>/src/shared/config/error-handler.ts` - Error handler pattern
...
1. **Existing Jina-v3 Integration** (from Stage 2):
   ```bash
   Read: <project-package-path>/src/shared/embeddings/generate.ts
...
2. **Qdrant Client** (for vector operations):
   ```bash
   Read: <project-package-path>/src/shared/integrations/qdrant/client.ts
   Validate: Connection setup, error handling
   ```

3. **Summarization Service** (integration point):
   ```bash
   Read: <project-package-path>/src/orchestrator/services/summarization-service.ts
   Identify: Where to inject quality gate logic
   ```

4. **Error Handler Pattern** (for retry logic):
   ```bash
   Read: <project-package-path>/src/shared/config/error-handler.ts
...
**File**: `<project-package-path>/src/orchestrator/services/quality-validator.ts`
...
**File**: `<project-package-path>/src/orchestrator/services/summarization-service.ts`
...
**File**: `<project-package-path>/src/orchestrator/workers/stage-3-create-summary-worker.ts`
...
**File**: `<project-package-path>/tests/unit/quality-validator.test.ts`
...
```json
{
  ...
  "files_created": [
    {
      "path": "<project-package-path>/src/orchestrator/services/quality-validator.ts",
      "reason": "Quality validation service with Jina-v3 + cosine similarity",
      "timestamp": "2025-10-28T12:05:00Z"
    },
    {
      "path": "<project-package-path>/tests/unit/quality-validator.test.ts",
  ...
  "files_modified": [
    {
      "path": "<project-package-path>/src/orchestrator/services/summarization-service.ts",
    ...
    {
      "path": "<project-package-path>/src/orchestrator/workers/stage-3-create-summary-worker.ts",
