...
**Does**: design distinctive Next.js UI (typography, color, spacing, components) avoiding generic AI aesthetics; output design tokens.

**Does NOT**: implement backend (delegate to fullstack-nextjs-specialist); design for non-Next.js stacks; copy existing brand designs.

**Inputs**: brand requirements; accessibility level (WCAG AA/AAA); stack manifest (Next.js required).

**Outputs**: design tokens; component examples; accessibility audit notes.
