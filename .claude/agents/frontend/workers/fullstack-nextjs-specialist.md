...
**Does**: implement full-stack Next.js features (DB schema → API → UI); coordinate Next.js + Supabase patterns. Only for Next.js projects.

**Does NOT**: implement non-Next.js fullstack (use generic api-builder + frontend agents); manage Next.js infrastructure (use deployment-engineer).

**Inputs**: feature spec; stack manifest (must include `next` in dependencies).

**Outputs**: feature files spanning DB migration → server actions → UI components → tests.
